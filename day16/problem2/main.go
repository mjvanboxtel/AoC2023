...
func findLargest(grid [][]Tile) int {
	var largest int
	var heading Vector2
	for i, val := range []int{1, len(grid) - 2} {
		if i == 0 {
			heading = Vector2{
				x:    0,
				y:    1,
		...
		for j := 1; j < len(grid[val])-2; j++ {
			newGrid := copyGrid(grid)
			wg.Add(1)
			go energize(newGrid, &newGrid[val][j], heading)
			wg.Wait()
	...
	for i, val := range []int{1, len(grid[0]) - 2} {
		if i == 0 {
			heading = Vector2{
				x:    1,
				y:    0,
		...
		for j := 1; j < len(grid)-2; j++ {
			newGrid := copyGrid(grid)
			wg.Add(1)
			go energize(newGrid, &newGrid[j][val], heading)
			wg.Wait()
			score := scoreGrid(newGrid)
			if score > largest {
				largest = score
